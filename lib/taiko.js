...
async function _click(selector, options, ...args) {
  const elems = await handleRelativeSearch(
    await findElements(selector),
    args,
  );
...
module.exports.dragAndDrop = async (source, destination) => {
  ...
  let dest =
    isSelector(destination) ||
    isString(destination) ||
    isElement(destination)
      ? await findFirstElement(destination)
      : destination;
  let options = setClickOptions({});
  await doActionAwaitingNavigation(options, async () => {
    if (defaultConfig.headful) {
      await highlightElemOnAction(sourceElem.get());
      if (isElement(dest)) await highlightElemOnAction(dest.get());
    }
...
const dragAndDrop = async (options, sourceElem, dest) => {
  let sourcePosition = await domHandler.boundingBoxCenter(
    sourceElem.get(),
  );
...
module.exports.hover = async (selector, options = {}) => {
  ...
  await scrollTo(e);
  if (defaultConfig.headful) await highlightElemOnAction(e.get());
...
module.exports.focus = async (selector, options = {}) => {
  ...
  await doActionAwaitingNavigation(options, async () => {
    if (defaultConfig.headful)
      await highlightElemOnAction(
        (await findFirstElement(selector)).get(),
      );
...
module.exports.attach = async (filepath, to) => {
  ...
  const nodeId = (await findFirstElement(to)).get();
  if (defaultConfig.headful) await highlightElemOnAction(nodeId);
...
async function highlight(selector, ...args) {
  ...
  descEvent.emit(
    'success',
    'Highlighted the ' + description(elems[0], true),
  );
}
...
async function mouseAction(
  ...
  if (!actions.includes(selector)) {
    ...
    if (elem == null) {
      throw Error(
        'Please provide a valid selector, unable to find element',
      );
    }
    await scrollTo(elem);
...
const scroll = async (
  ...
  const nodeId = (await findFirstElement(e)).get();
  if (defaultConfig.headful) await highlightElemOnAction(nodeId);
...
module.exports.$ = (selector, ...args) => {
  ...
  return generateElementListWrapper(get, description);
};
...
module.exports.listItem = (attrValuePairs, ...args) => {
  ...
  const get = getElementGetter(
    selector,
    async () => await match(selector.label, 'li').elements(0, 0),
    '//li',
  );
...
function fileField(attrValuePairs, _options = {}, ...args) {
  ...
  const value = async function() {
    return (
      await evaluate(this, function getvalue() {
        return this.value;
      })
    ).value;
  };

  const basicElementWrapper = generateElementListWrapper(
    get,
    description,
    {
      value: value,
...
function dropDown(attrValuePairs, ...args) {
  ...
  const select = async function(value) {
...
module.exports.checkBox = (attrValuePairs, _options, ...args) => {
  ...
  const isChecked = async function() {
    var val = (
      await evaluate(this, function getvalue() {
        return this.checked;
      })
...
module.exports.radioButton = (attrValuePairs, _options, ...args) => {
  ...
  const isSelected = async function() {
    var val = (
      await evaluate(this, function getvalue() {
        return this.checked;
      })
...
module.exports.evaluate = async (
  ...
  if (isFunction(selector)) {
    ...
    nodeId = (await findFirstElement(selector)).get();
  }
  if (defaultConfig.headful) await highlightElemOnAction(nodeId);
 ...
 * @param {number} [options.retryInterval = 1000 ] Option to modify delay time in milliseconds to retry the search of element existence.
 ...
 * @property @private {function} elements - DOM element getter. Implicitly wait for the element to appears with timeout of 10 seconds. Returns a list of elements found.
