...
const { $$xpath, waitAndGetElements } = require('./elementSearch');
...
const generateElementListWrapper = function(
  elementGetFn,
  elementDescription,
  customElementFns = {},
) {
  return {
    get: async function(retryInterval, retryTimeout) {
    ...
    text: async function() {
      const elems = await this.elements();
      if (!elems[0]) throw new Error(`${this.description} not found`);
      return await elems[0].text();
    },
    elements: waitAndGetElements(
      elementGetFn,
      elementDescription,
      customElementFns,
    ),
